...
final class GrapheneBrowserDebugScreen extends Screen {
    ...
    private static final BrowserOptions BROWSER_OPTIONS = BrowserOptions.builder()
    ...
    protected void init() {
        ...
        backButton =
                addRenderableWidget(browserNavigationButton(8, controlsY, Component.literal("<-"), webView::goBack));
        addRenderableWidget(browserNavigationButton(38, controlsY, Component.literal("R"), webView::reload));
        forwardButton = addRenderableWidget(
                browserNavigationButton(68, controlsY, Component.literal("->"), webView::goForward));
        addRenderableWidget(Button.builder(Component.literal("DevTools"), ignored -> openDevTools())
                .bounds(98, controlsY, 66, 20)
                .build());
        urlBox =
                addRenderableWidget(new EditBox(font, 168, controlsY, Math.max(1, width - 176), 20, Component.empty()));
    ...
    private Button browserNavigationButton(int x, int y, Component message, Runnable navigationAction) {
