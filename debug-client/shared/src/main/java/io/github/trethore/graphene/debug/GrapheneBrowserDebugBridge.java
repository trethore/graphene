...
final class GrapheneBrowserDebugBridge implements AutoCloseable {
    ...
    private void registerHandlers() {
        subscriptions.add(bridge.onRequest(
                "debug:echo", (channel, payload) -> CompletableFuture.completedFuture(echoResponse(payload))));
        subscriptions.add(bridge.onRequest(
                "debug:sum", (channel, payload) -> CompletableFuture.completedFuture(sumResponse(payload))));
        subscriptions.add(
                bridge.onRequest("debug:tests:run", (channel, payload) -> GrapheneDebugTestRunner.runAllTestsAsJson()));
        subscriptions.add(
                bridge.onRequest("debug:bridge:trigger-java-to-js", (channel, payload) -> javaToJsRoundTrip(payload)));
    }
