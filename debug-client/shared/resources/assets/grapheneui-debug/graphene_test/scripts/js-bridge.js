(function () {
	...
	function onJavaToJsRequest(payload) {
		...
		appendLogLine(
			"JS -> Java response debug:bridge:java-request",
			responsePayload,
		);
	...
	function triggerJavaToJsRoundTrip() {
		...
		currentBridge
			.request(CHANNEL_JAVA_TO_JS_TRIGGER, payload)
			.then(function (responsePayload) {
				appendLogLine(
					"JS -> Java request debug:bridge:trigger-java-to-js",
					responsePayload,
				);
			})
			.catch(function (error) {
	...
	function installHandlers() {
		document
			.getElementById("send-event")
			.addEventListener("click", sendEventToJava);
		document
			.getElementById("send-echo")
			.addEventListener("click", sendEchoRequest);
		document
			.getElementById("send-sum")
			.addEventListener("click", sendSumRequest);
		document
			.getElementById("run-java-to-js")
			.addEventListener("click", triggerJavaToJsRoundTrip);
	}
