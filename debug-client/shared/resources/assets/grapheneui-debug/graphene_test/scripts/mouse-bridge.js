(function () {
	...
	function renderSnapshot(mouseApi) {
		...
		const lastEvent = snapshot.lastEvent
			? "button=" +
				snapshot.lastEvent.button +
				", pressed=" +
				snapshot.lastEvent.pressed
			: "none";
	...
	function renderMouseEvent(mouseApi, eventPayload) {
		...
		statusElement().innerHTML =
			'<span class="ok">Bridge connected:</span> button ' +
			eventPayload.button +
			" " +
			action;
	...
	function connectMouseBridgeWhenAvailable() {
		...
		if (
			!mouseApi ||
			typeof mouseApi.on !== "function" ||
			typeof mouseApi.snapshot !== "function"
		) {
