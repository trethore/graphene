...
function quoted(value) {
    return value === undefined || value === null ? "" : JSON.stringify(String(value));
}
...
function modifierNames(event) {
    ...
    const modifierStates = ["AltGraph", "CapsLock", "NumLock", "ScrollLock", "Fn", "FnLock", "Symbol", "SymbolLock"];
...
function createRecord(event) {
    ...
    return {
        ...
        type: event.type,
        keyOrData: isInputEvent || event.type.startsWith("composition") ? quoted(event.data) : quoted(event.key),
        codeOrInputType: isInputEvent ? valueOrEmpty(event.inputType) : valueOrEmpty(event.code),
...
function buildReport() {
    const header = [
        "#", "type", "key/data", "code/inputType", "location", "which", "keyCode", "charCode",
        "modifiers/flags", "target", "activeElement", "timeStamp", "textAfterEvent"
    ].join("\t");
    return [
