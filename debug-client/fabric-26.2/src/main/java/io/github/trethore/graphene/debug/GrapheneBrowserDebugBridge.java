...
final class GrapheneBrowserDebugBridge implements AutoCloseable {
    ...
    private void registerHandlers() {
        subscriptions.add(bridge.onRequest(
                "debug:echo", (_, payload) -> CompletableFuture.completedFuture(echoResponse(payload))));
        subscriptions.add(
                bridge.onRequest("debug:sum", (_, payload) -> CompletableFuture.completedFuture(sumResponse(payload))));
        subscriptions.add(bridge.onRequest("debug:tests:run", (_, _) -> GrapheneDebugTestRunner.runAllTestsAsJson()));
        subscriptions.add(
                bridge.onRequest("debug:bridge:trigger-java-to-js", (_, payload) -> javaToJsRoundTrip(payload)));
    }
    ...
    private CompletableFuture<String> javaToJsRoundTrip(String payload) {
        ...
        return bridge.request("debug:bridge:java-request", event.toString(), REQUEST_TIMEOUT)
                .thenApply(response -> {
