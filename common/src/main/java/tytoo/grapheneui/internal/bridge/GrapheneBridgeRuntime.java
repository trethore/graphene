...
public final class GrapheneBridgeRuntime {
    ...
    private final Map<Object, GrapheneBridgeEndpoint> endpointsByBrowser = new IdentityHashMap<>();
    ...
    public GrapheneBridgeRuntime(GrapheneBridgeOptions options) {
        this(options, GrapheneMainThreadExecutor.DIRECT);
    }

    public GrapheneBridgeRuntime(GrapheneMainThreadExecutor mainThreadExecutor) {
        this(GrapheneBridgeOptions.defaults(), mainThreadExecutor);
    }

    public GrapheneBridgeRuntime(GrapheneBridgeOptions options, GrapheneMainThreadExecutor mainThreadExecutor) {
        this.options = Objects.requireNonNull(options, "options");
        this.mainThreadExecutor = Objects.requireNonNull(mainThreadExecutor, "mainThreadExecutor");
    }

    private static int browserIdentifier(Object browser) {
        try {
            if (browser instanceof GrapheneBridgeBrowser bridgeBrowser) {
                return bridgeBrowser.getIdentifier();
            }

            if (browser instanceof CefBrowser cefBrowser) {
    ...
    public GrapheneBridge attach(GrapheneBridgeBrowser browser) {
        Objects.requireNonNull(browser, BROWSER_NAME);

        GrapheneBridgeEndpoint previousEndpoint;
        GrapheneBridgeEndpoint newEndpoint = new GrapheneBridgeEndpoint(browser, options, mainThreadExecutor);
        synchronized (lock) {
    ...
    private GrapheneBridgeEndpoint endpointLocked(CefBrowser browser) {
        ...
        for (Map.Entry<Object, GrapheneBridgeEndpoint> entry : endpointsByBrowser.entrySet()) {
    ...
    private void cacheEndpointByIdentifierLocked(Object browser, GrapheneBridgeEndpoint endpoint) {
        int browserIdentifier = browserIdentifier(browser);
        if (browserIdentifier >= MIN_BROWSER_IDENTIFIER) {
