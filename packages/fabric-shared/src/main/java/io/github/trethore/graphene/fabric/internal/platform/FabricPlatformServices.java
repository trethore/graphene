...
import io.github.trethore.graphene.internal.platform.GrapheneFileDialogFilter;
...
import io.github.trethore.graphene.minecraft.internal.platform.GlfwNativeWindowHandle;
...
public final class FabricPlatformServices {
    ...
    private static GrapheneLifecycle lifecycle() {
        return new GrapheneLifecycle() {
            ...
            public void onStarted(Runnable action) {
                ClientLifecycleEvents.CLIENT_STARTED.register(ignored -> action.run());
            }

            @Override
            public void onStopping(Runnable action) {
                ClientLifecycleEvents.CLIENT_STOPPING.register(ignored -> action.run());
            }
        };
    }

    private static GrapheneTaskExecutor mainThreadExecutor() {
        return MinecraftReferences::execute;
    }
            ...
            public String resolveModId(Class<?> anchorClass) {
                ...
                for (ModContainer modContainer : FabricLoader.getInstance().getAllMods()) {
                    ...
                    if (resolvedModId != null && !resolvedModId.equals(candidateModId)) {
                        throw new IllegalStateException("Anchor class belongs to multiple mod containers: "
                                + resolvedModId
                                + " and "
                                + candidateModId);
                    }
        ...
        return new GrapheneStartupPresenter() {
            ...
            public void update(String stage, double progress) {
                ...
                MinecraftReferences.execute(() -> {
                    if (MinecraftReferences.overlay() == null || MinecraftReferences.overlay() == overlay) {
    ...
    private static BrowserContextMenuPresenter contextMenuPresenter() {
        return request -> {
            if (MinecraftReferences.screen() instanceof GrapheneScreenBridgeSupport bridge) {
    ...
    private static String showFileDialog(BrowserFileDialogPresenter.Request request) {
        ...
        return switch (request.mode()) {
            case OPEN_FOLDER -> TinyFileDialogs.tinyfd_selectFolderDialog(title, request.defaultFilePath());
            case OPEN_FILE, OPEN_MULTIPLE_FILES, SAVE_FILE -> showFileSelectionDialog(request, title);
        };
    }

    private static String showFileSelectionDialog(BrowserFileDialogPresenter.Request request, String title) {
        GrapheneFileDialogFilter filter = GrapheneFileDialogFilter.from(request.filters());
    ...
    private static BrowserJsDialogPresenter.Result showJsDialog(BrowserJsDialogPresenter.Request request) {
    ...
    private static BrowserJsDialogPresenter.Result showPrompt(String title, String message, String defaultPrompt) {
    ...
    private static BrowserJsDialogPresenter.Result showMessageDialog(
        ...
        boolean accepted =
                TinyFdMessageBoxAdapter.show(title, message, alert ? "ok" : "yesno", alert ? "info" : "question");
        return accepted ? BrowserJsDialogPresenter.Result.accept() : BrowserJsDialogPresenter.Result.cancel();
    }
}
