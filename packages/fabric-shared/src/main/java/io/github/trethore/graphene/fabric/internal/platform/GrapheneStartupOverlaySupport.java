...
final class GrapheneStartupOverlaySupport {
    ...
    void render(GrapheneGuiGraphics graphics) {
        int centerX = graphics.graphene$width() / 2;
        int barLeft = centerX - BAR_WIDTH / 2;
        int barTop = graphics.graphene$height() / 2;
        graphics.graphene$nextStratum();
        graphics.graphene$fill(0, 0, graphics.graphene$width(), graphics.graphene$height(), BACKGROUND);
        graphics.graphene$centeredText(
                MinecraftReferences.font(),
                Component.literal("Graphene: " + progress.displayStage()),
                centerX,
                barTop - 22,
                0xFFFFFFFF);
        graphics.graphene$fill(barLeft, barTop, barLeft + BAR_WIDTH, barTop + BAR_HEIGHT, BAR_OUTLINE);
        ...
        if (fillWidth > 0) {
            graphics.graphene$fill(barLeft + 1, barTop + 1, barLeft + 1 + fillWidth, barTop + BAR_HEIGHT - 1, BAR_FILL);
        }
    }
}
