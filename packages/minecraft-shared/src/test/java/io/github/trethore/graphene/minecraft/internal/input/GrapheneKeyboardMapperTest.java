...
class GrapheneKeyboardMapperTest {
    ...
    void mapsKeyLocationsSeparatelyFromModifierState() {
        BrowserKeyInput keypad = GrapheneKeyboardMapper.map(
                GLFW.GLFW_KEY_KP_ADD, 78, true, GLFW.GLFW_MOD_NUM_LOCK, Set.of(BrowserModifier.NUM_LOCK));
        BrowserKeyInput leftControl = GrapheneKeyboardMapper.map(
                GLFW.GLFW_KEY_LEFT_CONTROL, 29, true, GLFW.GLFW_MOD_CONTROL, Set.of(BrowserModifier.CONTROL));
