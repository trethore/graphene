package io.github.trethore.graphene.minecraft.internal.input;
...
public final class GrapheneKeyboardMapper {
    ...
    public static BrowserKeyInput map(
            int keyCode, int scanCode, boolean pressed, int glfwModifiers, Set<BrowserModifier> modifiers) {
        BrowserKey key = key(keyCode);
        BrowserKeyLocation location = location(keyCode);
        BrowserRawKeyMetadata rawMetadata = new BrowserRawKeyMetadata(
                BrowserKeyPlatform.current(), Math.max(scanCode, 0), layoutCodePoint(keyCode, scanCode, glfwModifiers));
        return new BrowserKeyInput(
                pressed ? BrowserKeyAction.PRESS : BrowserKeyAction.RELEASE, key, location, modifiers, rawMetadata);
    }
    ...
    static BrowserKeyLocation location(int keyCode) {
        ...
        return switch (keyCode) {
            case GLFW.GLFW_KEY_LEFT_SHIFT,
                    ...
                    GLFW.GLFW_KEY_LEFT_SUPER -> BrowserKeyLocation.LEFT;
            case GLFW.GLFW_KEY_RIGHT_SHIFT,
                    GLFW.GLFW_KEY_RIGHT_CONTROL,
                    GLFW.GLFW_KEY_RIGHT_ALT,
                    GLFW.GLFW_KEY_RIGHT_SUPER -> BrowserKeyLocation.RIGHT;
