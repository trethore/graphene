...
final class GrapheneGlobalConfigResolver {
    ...
    static GrapheneGlobalConfig resolve(Map<String, GrapheneConfig> consumerConfigs) {
        ...
        for (Map.Entry<String, GrapheneConfig> consumer : consumers) {
            ...
            GrapheneGlobalConfig config =
                    Objects.requireNonNull(consumer.getValue(), owner).global();
            config.browserRuntimePath()
        ...
        Path runtimePath = compatibleValue(
                runtimePaths, GrapheneGlobalConfigConflictException.Setting.BROWSER_RUNTIME_PATH, Path::toString);
        GrapheneRemoteDebugConfig remoteDebug = compatibleValue(
                remoteDebugConfigs,
                GrapheneGlobalConfigConflictException.Setting.REMOTE_DEBUGGING,
                GrapheneGlobalConfigResolver::describeRemoteDebugging);
        BrowserFileAccessPolicy fileAccessPolicy = compatibleValue(
                fileAccessPolicies,
    ...
    private static <T> T compatibleValue(
        ...
        boolean conflict = contributions.stream()
                .skip(1)
                .anyMatch(contribution -> !Objects.equals(selectedValue, contribution.value()));
        if (conflict) {
            List<GrapheneGlobalConfigConflictException.Contribution> reportedContributions = contributions.stream()
                    .map(contribution -> new GrapheneGlobalConfigConflictException.Contribution(
