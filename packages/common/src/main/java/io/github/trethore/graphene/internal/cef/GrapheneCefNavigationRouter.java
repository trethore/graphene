...
final class GrapheneCefNavigationRouter {
    ...
    GrapheneCefNavigationRouter(GrapheneTaskExecutor mainThreadExecutor, GrapheneExternalBrowser externalBrowser) {
    ...
    boolean onMainFrameNavigation(
        ...
        BrowserNavigationPolicy.Request request = request(
                session,
    ...
    private boolean apply(
            BrowserSession session, BrowserNavigationPolicy.Request request, boolean preserveOriginalNavigation) {
    ...
    private void navigate(BrowserSession session, String targetUrl) {
        ...
        schedule("navigate session to", targetUrl, () -> {
            if (!session.isClosed()) {
    ...
    private static Optional<BrowserNavigationPolicy.Frame> frame(CefFrame frame) {
        ...
        return Optional.of(new BrowserNavigationPolicy.Frame(
                value(frame.getIdentifier()), value(frame.getName()), value(frame.getURL()), frame.isMain()));
    }
