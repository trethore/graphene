...
final class GrapheneCefInputTranslator {
    ...
    static CefKeyEvent key(BrowserKeyInput input) {
        ...
        return new CefKeyEvent(
                input.action() == BrowserKeyAction.PRESS ? CefKeyEvent.KEYEVENT_RAWKEYDOWN : CefKeyEvent.KEYEVENT_KEYUP,
    ...
    static CefKeyEvent[] text(BrowserTextInput input, BrowserKeyInput originatingKey) {
        ...
        int eventModifiers = modifiers(input.modifiers())
                | (originatingKey == null ? EventFlags.EVENTFLAG_NONE : locationModifier(originatingKey.location()));
        CefKeyEvent[] events = new CefKeyEvent[text.length()];
        for (int index = 0; index < text.length(); index++) {
            char character = text.charAt(index);
            char unmodifiedCharacter = key != null && text.length() == 1 && key.unmodifiedCharacter() != 0
                    ? key.unmodifiedCharacter()
                    : character;
            events[index] = new CefKeyEvent(
                    CefKeyEvent.KEYEVENT_CHAR,
                    eventModifiers,
                    key == null || key.platform() == BrowserKeyPlatform.WINDOWS ? character : key.windowsKeyCode(),
