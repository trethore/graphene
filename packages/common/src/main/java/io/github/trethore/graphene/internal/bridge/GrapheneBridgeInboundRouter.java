...
final class GrapheneBridgeInboundRouter {
    ...
    private void handleRequest(GrapheneBridgePacket packet, BridgeQueryCallback callback) {
        if (packet.id == null || packet.id.isBlank()) {
            callback.success(codec.createErrorResponseJson(
    ...
    private void handleRequestResponse(
            GrapheneBridgePacket packet, BridgeQueryCallback callback, CompletableFuture<String> responseFuture) {
        if (responseFuture == null) {
            callback.success(codec.createSuccessResponseJson(packet.id, packet.channel, JsonNull.INSTANCE));
            LOGGER.debug(
                    "Bridge request completed with null response future id={} channel={}", packet.id, packet.channel);
            return;
        }
