...
public final class GrapheneBridgeRuntime {
    ...
    private final Map<BridgeBrowser, GrapheneBridgeEndpoint> endpointsByBrowser = new IdentityHashMap<>();
    ...
    public GrapheneBridge attach(BridgeBrowser browser, GrapheneBridgeExposureConfig exposureConfig) {
        ...
        GrapheneBridgeEndpoint newEndpoint = new GrapheneBridgeEndpoint(browser, taskExecutor, exposureConfig);
        synchronized (lock) {
    ...
    public boolean onQuery(BridgeBrowser browser, BridgeFrame frame, String request, BridgeQueryCallback callback) {
        GrapheneBridgeEndpoint endpoint = endpoint(browser);
        if (endpoint == null) {
            LOGGER.debug("Bridge query ignored because endpoint is missing browserId={}", browserIdentifier(browser));
            return false;
        }
    ...
    private void cacheEndpointByIdentifierLocked(BridgeBrowser browser, GrapheneBridgeEndpoint endpoint) {
        int browserIdentifier = browserIdentifier(browser);
        if (browserIdentifier >= MIN_BROWSER_IDENTIFIER) {
