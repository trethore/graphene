...
public interface BrowserBridgePolicy {
    ...
    static BrowserBridgePolicy initialOrigin() {
        return request -> request.origin().isPresent() && request.origin().equals(request.initialOrigin())
                ? Decision.ALLOW
                : Decision.DENY;
    }
    ...
    static BrowserBridgePolicy allowOrigins(Set<BrowserBridgeOrigin> origins) {
        Set<BrowserBridgeOrigin> allowedOrigins = Set.copyOf(Objects.requireNonNull(origins, "origins"));
        return request ->
                request.origin().filter(allowedOrigins::contains).isPresent() ? Decision.ALLOW : Decision.DENY;
    }
