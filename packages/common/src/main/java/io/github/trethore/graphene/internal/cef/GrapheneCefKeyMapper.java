...
final class GrapheneCefKeyMapper {
    ...
    static ResolvedKey resolve(BrowserKeyInput input) {
        ...
        BrowserKeyPlatform platform = rawMetadata == null ? BrowserKeyPlatform.current() : rawMetadata.platform();
        ...
        int nativeKeyCode = nativeKeyCode(input.key(), input.location(), input.action(), platform, scanCode);
        ...
        return new ResolvedKey(
                platform, windowsKeyCode, nativeKeyCode, systemKey, character, unmodifiedCharacter, scanCode);
    }
    ...
    private static char layoutCharacter(BrowserKeyInput input, BrowserRawKeyMetadata rawMetadata) {
        ...
        return input.modifiers().contains(BrowserModifier.SHIFT) ? Character.toUpperCase(character) : character;
    }
            ...
            case WINDOWS ->
                scanCode == 0
                        ...
                        : CefKeyEvent.buildWindowsNativeKeyCode(
                                scanCode, isExtendedWindowsKey(key, location), action == BrowserKeyAction.RELEASE);
    ...
    private static long resolveScanCode(
            BrowserKey key, BrowserKeyLocation location, BrowserKeyPlatform platform, long suppliedScanCode) {
    ...
    private static char unmodifiedCharacter(BrowserKey key, BrowserKeyPlatform platform, char layoutCharacter) {
        return switch (platform) {
            case WINDOWS -> 0;
            case MACOS ->
                switch (key) {
    ...
    private static char character(BrowserKey key, char unmodifiedCharacter, boolean control, boolean shift) {
        if (!control) {
    ...
    private static boolean isExtendedWindowsKey(BrowserKey key, BrowserKeyLocation location) {
        if ((key == BrowserKey.CONTROL || key == BrowserKey.ALT) && location == BrowserKeyLocation.RIGHT) {
            return true;
        }
