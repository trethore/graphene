...
final class GrapheneBridgeRequestLifecycle {
    private static final Logger LOGGER = LoggerFactory.getLogger(GrapheneBridgeRequestLifecycle.class);
    ...
    CompletableFuture<String> request(String channel, String payloadJson, Duration timeout) {
        ...
        try {
            String outboundJson = codec.createOutboundPacketJson(
                    GrapheneBridgeProtocol.KIND_REQUEST, requestId, channel, codec.parsePayloadJson(payloadJson));
    ...
    void handleResponse(GrapheneBridgePacket packet) {
        ...
        if (Boolean.FALSE.equals(packet.ok)) {
            ...
            String errorMessage = packet.error == null || packet.error.message == null
                    ? "Bridge request failed"
                    : packet.error.message;
            pendingRequests.completeFailure(
                    packet.id, new GrapheneBridgeRequestException(errorCode, errorMessage, packet.id, packet.channel));
            LOGGER.debug(
