...
final class GrapheneCefDownloadRegistry implements AutoCloseable {
    ...
    private static final Pattern INVALID_FILE_NAME_CHARACTERS = Pattern.compile("[\\x00-\\x1F<>:\"/\\\\|?*]");
    ...
    private final GrapheneListenerList<BrowserDownloadListener> listeners = new GrapheneListenerList<>();
    ...
    boolean onBeforeDownload(CefDownloadItem item, String suggestedName, CefBeforeDownloadCallback callback) {
    ...
    private void finish(int cefIdentifier, ActiveDownload download, BrowserDownloadState terminalState) {
        synchronized (this) {
    ...
    private void publish(BrowserDownload download) {
        listeners.dispatch(listener -> listener.onDownloadChanged(download), LOGGER, "browser download listener");
    }
    ...
    private static String sanitizeFileName(String suggestedName) {
        ...
        String sanitized = stripTrailingDotsAndSpaces(
    ...
    private static final class ActiveDownload implements BrowserDownloadControl {
        ...
        private ActiveDownload(BrowserDownloadId id, CefDownloadItem item, String suggestedFileName) {
            this.metadata = new GrapheneCefDownloadSnapshot.Metadata(
                    id,
