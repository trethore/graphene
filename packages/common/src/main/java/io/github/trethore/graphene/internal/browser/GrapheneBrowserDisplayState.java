...
public final class GrapheneBrowserDisplayState implements AutoCloseable {
    ...
    private final GrapheneListenerList<BrowserTitleListener> titleListeners = new GrapheneListenerList<>();
    private final GrapheneListenerList<BrowserUrlListener> urlListeners = new GrapheneListenerList<>();
    ...
    public Runnable updateTitle(String title) {
        ...
        return notification(
                titleListeners, BrowserTitleListener::onTitleChanged, normalizedTitle, "browser title-change listener");
    }
    ...
    public Runnable updateUrl(String url) {
        ...
        return notification(
                urlListeners, BrowserUrlListener::onUrlChanged, normalizedUrl, "browser URL-change listener");
    }

    public Runnable consoleMessage(BrowserConsoleSeverity severity, String message, String source, int line) {
        synchronized (this) {
            if (closed || consoleMessageListeners.isEmpty()) {
                return null;
            }
        }
        BrowserConsoleMessage consoleMessage = new BrowserConsoleMessage(
    ...
    private static <T, U> Runnable notification(
            GrapheneListenerList<T> listeners, BiConsumer<T, U> callback, U event, String listenerDescription) {
        if (listeners.isEmpty()) {
            return null;
        }
        return () -> listeners.dispatch(listener -> callback.accept(listener, event), LOGGER, listenerDescription);
    }
