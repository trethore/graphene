...
final class GrapheneCefRequestHandler extends CefRequestHandlerAdapter {
    ...
    public boolean onBeforeBrowse(
            CefBrowser browser, CefFrame frame, CefRequest request, boolean userGesture, boolean isRedirect) {
        if (request == null) {
            return false;
        }
        return navigationRouter.onMainFrameNavigation(browser, frame, request.getURL(), userGesture, isRedirect);
    }

    @Override
    public boolean onOpenURLFromTab(CefBrowser browser, CefFrame frame, String targetUrl, boolean userGesture) {
        return navigationRouter.onOpenFromTab(browser, frame, targetUrl, CefWindowOpenDisposition.UNKNOWN, userGesture);
    }
    ...
    public boolean onOpenURLFromTab(
            ...
            boolean userGesture) {
        return navigationRouter.onOpenFromTab(browser, frame, targetUrl, targetDisposition, userGesture);
    }
}
