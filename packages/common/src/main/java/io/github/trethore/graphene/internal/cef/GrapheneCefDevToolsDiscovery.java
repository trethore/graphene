...
final class GrapheneCefDevToolsDiscovery {
    ...
    static CompletionStage<DevToolsPageTarget> targetFor(GrapheneCefBrowserSession session, int port) {
        ...
        return pageTargets(port).thenApply(targets -> {
            DevToolsPageTarget target = resolveTarget(targets, boundTargetId, sessionUrl, sessionTitle);
    ...
    static List<DevToolsPageTarget> parsePageTargets(String responseBody, URI discoveryUri) {
        try {
            ...
            if (!response.isJsonArray()) {
                throw new DevToolsDiscoveryException("Remote DevTools target discovery response must be a JSON array");
            }
            ...
            for (JsonElement targetElement : targetArray) {
                ...
                DevToolsPageTarget target = new DevToolsPageTarget(
    ...
    static DevToolsPageTarget resolveTarget(
            List<DevToolsPageTarget> targets, String boundTargetId, String sessionUrl, String sessionTitle) {
        if (boundTargetId != null) {
        ...
        List<DevToolsPageTarget> urlMatches = targets.stream()
                .filter(target -> target.url().equals(sessionUrl))
                .toList();
        ...
        if (!sessionTitle.isBlank()) {
            List<DevToolsPageTarget> titleMatches = urlMatches.stream()
                    .filter(target -> target.title().equals(sessionTitle))
                    .toList();
    ...
    private static String optionalString(JsonObject object, String name) {
        ...
        if (!value.isJsonPrimitive() || !value.getAsJsonPrimitive().isString()) {
            throw new DevToolsDiscoveryException("Remote DevTools target field " + name + " must be a string");
        }
    ...
    private static String requiredString(JsonObject object, String name) {
        ...
        if (value.isBlank()) {
            throw new DevToolsDiscoveryException("Remote DevTools target field " + name + " must not be blank");
        }
        return value;
    }
    ...
    private static String requiredStringAllowBlank(JsonObject object, String name) {
        ...
        if (value == null) {
            throw new DevToolsDiscoveryException("Remote DevTools target is missing required field " + name);
        }
        return value;
    }
}
