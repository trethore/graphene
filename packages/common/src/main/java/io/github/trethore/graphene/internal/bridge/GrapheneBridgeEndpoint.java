...
public final class GrapheneBridgeEndpoint implements GrapheneBridge {
  ...
  public GrapheneBridgeEndpoint(BridgeBrowser browser) {
    this(browser, GrapheneBridgeOptions.defaults(), GrapheneTaskExecutor.direct());
  }

  GrapheneBridgeEndpoint(
      BridgeBrowser browser, GrapheneBridgeOptions options, GrapheneTaskExecutor taskExecutor) {
    ...
    this.inboundRouter =
        new GrapheneBridgeInboundRouter(
            codec, handlers, requestLifecycle, this::onBridgeReady, taskExecutor);
  ...
  public boolean handleQuery(String requestJson, BridgeQueryCallback callback) {
    if (closed.get()) {
      return false;
    }
