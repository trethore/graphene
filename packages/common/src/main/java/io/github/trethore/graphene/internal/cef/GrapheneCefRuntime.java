...
public final class GrapheneCefRuntime implements GrapheneBrowserRuntime {
    ...
    public synchronized void initialize(GrapheneGlobalConfig config) {
        ...
        builder.setProgressHandler(
                (state, percent) -> startupPresenter.update(state.name(), percent < 0 ? -1.0 : percent / 100.0));
        try {
    ...
    private static CefApp buildApp(CefAppBuilder builder) {
        try {
            ...
            throw new IllegalStateException("Interrupted while initializing Graphene browser runtime", exception);
    ...
    public CompletionStage<DevToolsPageTarget> devToolsTargetFor(BrowserSession session) {
        ...
        synchronized (this) {
            ...
            if (!(session instanceof GrapheneCefBrowserSession candidate) || !sessions.contains(candidate)) {
    ...
    public synchronized BrowserSession createSession(
        ...
        GrapheneCefBrowserSession session = new GrapheneCefBrowserSession(
                client,
                url,
                options,
                width,
    ...
    private static void disposePartial(CefApp activeApp, List<GrapheneCefBrowserSession> closingSessions) {
        try {
            if (activeApp != null) {
                activeApp.dispose();
            }
    ...
    private static void killJcefProcesses() {
        List<ProcessHandle> processes = ProcessHandle.current()
                .descendants()
                .filter(GrapheneCefRuntime::isJcefProcess)
                .toList();
