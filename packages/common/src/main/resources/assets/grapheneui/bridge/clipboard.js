...
async function grapheneClipboardPayloadFromItems(items) {
    ...
    for (const item of items ?? []) {
        for (const type of item.types ?? []) {
            if (type !== "text/plain" && type !== "text/html" && type !== "image/png") {
                continue;
            }
    ...
    globalThis.setTimeout(function () {
        clipboard.read()
            ...
            .catch(function (error) {
                grapheneClipboardReportSuppressedError("Failed to read browser clipboard formats", error);
            });
    }, 0);
}
...
function grapheneClipboardOnCopy(event) {
    ...
    grapheneClipboardEmit(bridge, payload)
        .catch(function (error) {
            grapheneClipboardReportSuppressedError("Failed to write host clipboard", error);
        });
...
function grapheneClipboardInsertRichContent(target, payload) {
    const editableTarget = target?.closest?.("[contenteditable]:not([contenteditable='false'])");
    if (!editableTarget) {
        return false;
    }
    ...
    if (!grapheneClipboardDispatchInput(editableTarget, "beforeinput", payload.text ?? "")) {
        return true;
    }
...
function grapheneClipboardCreateHtmlFragment(html) {
    ...
    template.content.querySelectorAll("script, iframe, object, embed, meta").forEach(function (element) {
        element.remove();
    });
...
function grapheneClipboardPatchWrites(bridge) {
    ...
    const synchronizedWriteText = function (text) {
        ...
        return nativeWriteText(normalizedText).then(function () {
            return grapheneClipboardEmit(bridge, {text: normalizedText, html: null, png: null});
        });
    };
    ...
    try {
        ...
        grapheneClipboardReportSuppressedError("Failed to synchronize navigator.clipboard.writeText", error);
    }
    ...
    try {
        ...
        grapheneClipboardReportSuppressedError("Failed to synchronize navigator.clipboard.write", error);
    }
}
...
function grapheneClipboardTryInstall() {
    ...
    const bridge = globalThis.grapheneBridge;
    if (!bridge || typeof bridge.on !== "function" || typeof bridge.emit !== "function") {
