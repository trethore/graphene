...
class GrapheneLoadEventBusTest {
    ...
    void dispatchesGrapheneOwnedEventsInOrder() {
        try (GrapheneLoadEventBus eventBus = new GrapheneLoadEventBus()) {
            ...
            BrowserLoadStarted started = new BrowserLoadStarted("app://assets/test", true, BrowserLoadTransition.LINK);
            BrowserLoadCompleted completed = new BrowserLoadCompleted("app://assets/test", true, OptionalInt.of(200));
            BrowserLoadFailed failed = new BrowserLoadFailed(
                    "app://assets/test", true, BrowserLoadFailureReason.UNKNOWN, "failed", OptionalInt.of(-2));
    ...
    void listenerFailureDoesNotPreventRemainingListeners() {
        try (GrapheneLoadEventBus eventBus = new GrapheneLoadEventBus()) {
            List<Object> receivedEvents = new ArrayList<>();
            eventBus.subscribe(new BrowserLoadListener() {
                @Override
