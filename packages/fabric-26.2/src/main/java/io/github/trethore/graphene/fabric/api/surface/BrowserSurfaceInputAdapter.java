...
public final class BrowserSurfaceInputAdapter implements AutoCloseable {
    ...
    public void mouseMoved(
        ...
        BrowserPointerAction action =
                pressedButton == BrowserPointerButton.NONE ? BrowserPointerAction.MOVE : BrowserPointerAction.DRAG;
        sendPointer(
                action, pressedButton, mouseX, mouseY, surfaceX, surfaceY, renderedWidth, renderedHeight, 0, modifiers);
    }
    ...
    public void key(int keyCode, int scanCode, boolean pressed, int modifiers) {
        ...
        surface.browser()
                .sendKeyInput(GrapheneKeyboardMapper.map(
    ...
    private boolean isSyntheticDuplicate(String text) {
        ...
        boolean duplicate = pendingSyntheticText.equals(text)
                && System.currentTimeMillis() - pendingSyntheticTimestamp <= SYNTHETIC_DUPLICATE_WINDOW_MILLIS;
        pendingSyntheticText = null;
        pendingSyntheticTimestamp = 0;
        return duplicate;
    }
    ...
    static GrapheneClipboardContent resolveClipboardContent(GrapheneClipboardContent richContent, String nativeText) {
        GrapheneClipboardContent validatedRichContent = Objects.requireNonNull(richContent, "richContent");
