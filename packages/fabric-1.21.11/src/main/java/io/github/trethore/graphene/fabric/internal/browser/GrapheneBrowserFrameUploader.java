...
final class GrapheneBrowserFrameUploader {
    ...
    void upload(GrapheneBrowserGpuTexture texture, BrowserFrame frame, boolean transparent) {
        ...
        if (shouldUploadFullFrame(frame, texture.canApplyDirtyRegions(frame.sequence()))) {
            uploadRegion(texture, pixels, rowStrideBytes, transparent, 0, 0, frame.width(), frame.height());
        } else {
            for (BrowserDirtyRegion region : frame.dirtyRegions()) {
                ...
                if (width > 0 && height > 0) {
                    uploadRegion(texture, pixels, rowStrideBytes, transparent, region.x(), region.y(), width, height);
                }
            }
        }
    ...
    private void uploadRegion(
        ...
        RenderSystem.getDevice()
                ...
                .writeToTexture(texture.texture(), converted, NativeImage.Format.RGBA, 0, 0, x, y, width, height);
    }
