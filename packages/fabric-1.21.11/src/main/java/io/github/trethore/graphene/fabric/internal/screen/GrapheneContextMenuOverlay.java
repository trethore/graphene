...
final class GrapheneContextMenuOverlay {
    ...
    GrapheneContextMenuOverlay(
        ...
        height = items.stream().mapToInt(GrapheneContextMenuOverlay::itemHeight).sum() + VERTICAL_PADDING * 2 + 2;
    ...
    void keyPressed(KeyEvent event) {
        switch (event.key()) {
            ...
            case GLFW.GLFW_KEY_ENTER, GLFW.GLFW_KEY_KP_ENTER -> selectKeyboardSelection(event.modifiers());
            default -> {
    ...
    private void select(BrowserContextMenuItem.Command command, int modifiers) {
        ...
        completion.complete(
                BrowserContextMenuPresenter.Result.select(command, GrapheneInputModifiers.fromGlfw(modifiers)));
    }
