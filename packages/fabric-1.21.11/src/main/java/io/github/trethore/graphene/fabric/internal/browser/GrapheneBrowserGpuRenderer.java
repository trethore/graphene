...
public final class GrapheneBrowserGpuRenderer implements AutoCloseable {
    ...
    public void render(
            GuiGraphics graphics, BrowserFrame frame, boolean transparent, int x, int y, int width, int height) {
        if (width <= 0 || height <= 0) {
            return;
        }
        ...
        graphics.guiRenderState.submitGuiElement(new BlitRenderState(
                transparent ? RenderPipelines.GUI_TEXTURED_PREMULTIPLIED_ALPHA : RenderPipelines.GUI_TEXTURED,
